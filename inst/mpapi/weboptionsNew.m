...
classdef weboptionsNew < handle

  ## -*- texinfo -*-
  ## @deftypefn  {} {@var{output} =} weboptionsNew ()
  ## @deftypefnx {} {@var{output} =} weboptionsNew (@var{name1}, @var{value1}, @dots{})
  ##
  ## Specify parameters for RESTful web services.
  ##
  ## @code{weboptionsNew} with no inputs returns a default @code{weboptionsNew} object
  ## to specify parameters for a request to a web service.  A @code{weboptionsNew}
  ...
  methods
    function f = weboptionsNew(varargin)
      if (rem (numel (varargin), 2) != 0)
        error ("weboptionsNew: invalid number of arguments");
      else
        ...
        if (! isempty (fieldnames (h)))
          ...
          error (["weboptionsNew: Undefined field " field]);
        endif
      endif
    endfunction
    ...
    function f = set.CharacterEncoding (f, value)
      if (! any (strcmpi (value, {"UTF-8", 'US-ASCII', "auto"})));
        error ("weboptionsNew: Invalid CharacterEncoding value");
      else
    ...
    function f = set.UserAgent (f, value)
      if (! ischar (value) && ! isrow (value));
        error ("weboptionsNew: UserAgent must be a string");
      else
        f.UserAgent = value;
      endif
    endfunction
    ...
    function f = set.Timeout (f, value)
      if (! isreal (value) || ! isscalar (value)
        ...
        error ("weboptionsNew: invalid Timeout value");
      else
        f.Timeout = value;
      endif
    endfunction
    ...
    function f = set.Username (f, value)
      if (! ischar (value) && ! isrow (value))
        error ("weboptionsNew: Username must be a string");
      else
        f.Username = value;
      endif
    endfunction
    ...
    function f = set.Password (f, value)
      if (! ischar (value) && ! isrow (value))
        error ("weboptionsNew: Password must be a string");
      else
        f.Password = value;
      endif
    endfunction
    ...
    function f = set.KeyName (f, value)
      if (! ischar (value) && ! isrow (value))
        error ("weboptionsNew: invalid KeyName value");
      else
        f.KeyName = value;
      endif
    endfunction
    ...
    function f = set.KeyValue (f, value)
      if (isempty (f.KeyName) && ! isempty (value))
        error ("weboptionsNew: field KeyName empty.  Cannot set KeyValue.");
      else
        f.KeyValue = value;
      endif
    endfunction
    ...
    function f = set.HeaderFields (f, value)
      if (! isempty (value))
        if (! iscellstr (value) || ! ismatrix (value))
          error ("weboptionsNew: HeaderFields must be array of strings or a cell array");
        % fix for flattened array in __restful_service__
        elseif (columns (value) != 2) && mod(numel(value),2) ~= 0
          error ("weboptionsNew: HeaderFields must be of size m-by-2");
        endif
      endif
    ...
    function f = set.ContentType (f, value)
      if (! isempty (value))
        if (! any (strcmpi (value, {"auto", "json", "text"})))
          error ("weboptionsNew: invalid ContentType value");
        endif
      endif
      f.ContentType = value;
    endfunction
    ...
    function f = set.RequestMethod (f, value)
      if (! isempty (value))
        if (! any (strcmpi (value, {"auto", "get", "put", "post",...
                                    "delete", "patch"})))
          error ("weboptionsNew: invalid RequestMethod value");
        endif
      endif
    ...
    function f = set.ArrayFormat (f, value)
      if (! isempty (value))
        if (! any (strcmpi (value, {"csv", "json", "php", "repeating"})))
          error ("weboptionsNew: invalid ArrayFormat value");
        endif
      endif
      f.ArrayFormat = value;
    endfunction
